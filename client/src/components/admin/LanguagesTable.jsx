...
const LanguagesTable = () => {
    ...
    useEffect(()=>{
        const filteredLanguage = languages.length >= 0 && languages.filter((language)=>{
            ...
            return language?.name?.toLowerCase().includes(searchLanguageByText.toLowerCase());
                ...
                <TableBody>
                    {
                        filterLanguage?.map((language) => (
                            <tr key={language._id}>
                                <TableCell>
                                    <Avatar>
                                        <AvatarImage src={language.logo}/>
                                    </Avatar>
                                </TableCell>
                                <TableCell>{language.languageName}</TableCell>
                                <TableCell>{language.createdAt.split("T")[0]}</TableCell>
